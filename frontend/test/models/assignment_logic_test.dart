...
void main() {
  group('StudentAssignmentLogic (Active Selection)', () {
    ...
    setUp(() {
      dummyPlan = Plan(
          id: 'dummy', name: 'Dummy', weeks: <PlanWeek>[], durationWeeks: 4);
    });
    ...
    test('returns null if no assignment is active', () {
      final a1 = StudentAssignment(
          id: '1', isActive: false, plan: dummyPlan, progress: {});
      expect([a1].activeAssignment, isNull);
    });

    test('returns the single active assignment', () {
      final a1 = StudentAssignment(
          id: '1', isActive: true, plan: dummyPlan, progress: {});
    ...
    test('falls back to first active if tie (no progress)', () {
      final a1 = StudentAssignment(
          id: '1', isActive: true, plan: dummyPlan, progress: {});
      final a2 = StudentAssignment(
          id: '2', isActive: true, plan: dummyPlan, progress: {});
  ...
  group('PlanTraversalLogic (Next Workout)', () {
    final day1 =
        PlanDay(id: 'd1', dayOfWeek: 1, order: 1, exercises: <PlanExercise>[]);
    final day2 =
        PlanDay(id: 'd2', dayOfWeek: 3, order: 2, exercises: <PlanExercise>[]);
    final week1 =
        PlanWeek(id: 'w1', weekNumber: 1, days: <PlanDay>[day1, day2]);
    final plan = Plan(
        id: 'p1',
        name: 'Test Plan',
    ...
    test('returns first day if no progress', () {
      final assign =
          StudentAssignment(id: 'a1', isActive: true, plan: plan, progress: {});
