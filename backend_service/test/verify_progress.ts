...
async function main() {
    ...
    try {
        ...
        const adminLogin = await login('admin@gym.com', 'admin123').catch(() => login('admin@gym.com', '123456'));
        ...
        if (exercises.length > 0) {
            ...
            const newEx = await post('/exercises', {
        ...
        const studUser = await post('/users', {
            ...
            age: 20,
            gender: 'F'
        }, profToken); // Profe creates student
        ...
        // Mark Day
        await patch('/plans/student/progress', {
        ...
        // Check Day
        if (myAssignment.progress.days && myAssignment.progress.days[dayId] && myAssignment.progress.days[dayId].completed) {
            console.log('   [PASS] Day marked as completed.');
        } else {
            console.error('   [FAIL] Day NOT marked as completed.', JSON.stringify(myAssignment.progress));
            throw new Error('Day verification failed');
        }

        // Check Exercise
        if (myAssignment.progress.exercises && myAssignment.progress.exercises[exId]) {
            console.log('   [PASS] Exercise marked as completed.');
        } else {
            console.error('   [FAIL] Exercise NOT marked as completed.', JSON.stringify(myAssignment.progress));
