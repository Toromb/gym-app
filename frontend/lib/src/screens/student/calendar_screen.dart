...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _fetchEvents() async {
    ...
    try {
      final executions =
          await context.read<PlanProvider>().fetchCalendar(start, end);
        ...
        Padding(
          ...
          child: GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 7,
                childAspectRatio: 0.85), // Closer to square, safe with Stack
        ...
        _isLoading
            ...
            : _selectedDay == null
                ? Padding(
                    ...
                    child: Center(
                        child: Text(
                            AppLocalizations.of(context)!.get('selectDay'))),
                  )
                : _buildEventList(),
      ],
    );
  ...
  Widget _buildEventList() {
    ...
    if (events.isEmpty) {
      return Padding(
        ...
        child: Center(
            child: Text(
                '${AppLocalizations.of(context)!.get('noWorkoutsOn')} ${DateFormat.yMMMd(AppLocalizations.of(context)!.locale.languageCode).format(_selectedDay!)}',
    ...
    return Container(
      ...
      child: Theme(
        data: Theme.of(context).copyWith(
            dividerColor: Colors.transparent), // Hide internal dividers
        child: ExpansionTile(
          ...
          children: execution.exercises.map((ex) {
            String metricText = '';
            // ... logic ...
            if (ex.timeSpent != null &&
                ex.timeSpent!.isNotEmpty &&
                ex.timeSpent != '0' &&
                ex.timeSpent != '00:00') {
              metricText =
                  '${AppLocalizations.of(context)!.get('time')}: ${ex.timeSpent}';
            } else if (ex.distanceCovered != null && ex.distanceCovered! > 0) {
              metricText =
                  '${AppLocalizations.of(context)!.get('distance')}: ${ex.distanceCovered} m';
            } else {
              metricText =
