...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  // Step 3: Goals
  String? _goal;
  final TextEditingController _goalDetailsController =
      TextEditingController(); // For Sport/Rehab
  String? _desiredFrequency;

  // Step 4: Injuries & Prefs
  final List<String> _injuries = [];
  final TextEditingController _injuryDetailsController =
      TextEditingController();
  final TextEditingController _preferencesController =
      TextEditingController(); // Only optional field
  ...
  void _nextPage() {
    if (_currentPage == 4) {
      if (_canLieDown == null || _canKneel == null || _injuries.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Por favor respondé todas las preguntas de salud.')));
        return;
      }
      _submit();
      return;
    ...
    if (_currentPage == 2) {
      if (_experience == null || _activityLevel == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text('Por favor completá todos los campos para continuar.')));
        return;
      }
    }
    ...
    if (_currentPage == 3) {
      if (_goal == null || _desiredFrequency == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Seleccioná tu objetivo y frecuencia.')));
        return;
      }
      if ((_goal == 'sport' || _goal == 'rehab') &&
          _goalDetailsController.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text('Por favor especificá detalles de tu deporte o lesión.')));
        return;
      }
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final dto = CreateOnboardingDto(
        goal: _goal!,
        goalDetails: _goalDetailsController.text.isNotEmpty
            ? _goalDetailsController.text
            : null,
        experience: _experience!,
        injuries: _injuries,
        injuryDetails: _injuryDetailsController.text.isNotEmpty
            ? _injuryDetailsController.text
            : null,
        ...
        birthDate: _birthDateController.text.isNotEmpty
            ...
            : null, // Assuming YYYY-MM-DD from picker
      ...
      await authProvider
          .refreshUser(); // Refresh user data to get Gender/Weight
      ...
      child: Column(
        ...
        children: [
          const Icon(Icons.waving_hand,
              size: 80, color: Colors.orange), // Friendly icon
          ...
          Row(
            children: [
              Expanded(
                ...
                decoration: const InputDecoration(
                    labelText: 'Altura (cm)', suffixText: 'cm'),
              )),
              ...
              Expanded(
                ...
                keyboardType:
                    const TextInputType.numberWithOptions(decimal: true),
                decoration: const InputDecoration(
                    labelText: 'Peso Inicial (kg)', suffixText: 'kg'),
              )),
              const SizedBox(width: 16),
              Expanded(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          const Text('¿Cuál es tu experiencia previa en gimnasios?',
          ...
          const SizedBox(height: 24),
          const Text('Nivel de actividad física actual',
              style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                    labelText:
                        _goal == 'sport' ? '¿Qué deporte?' : '¿Qué lesión?',
                    border: const OutlineInputBorder()),
              ),
            ),
          const SizedBox(height: 24),
          const Text('¿Con qué frecuencia planeas entrenar?',
              style: TextStyle(fontWeight: FontWeight.bold)),
          Wrap(
            spacing: 8,
            children: _frequencyOptions.entries.map((e) {
              ...
              return ChoiceChip(
                ...
                selected: selected,
                onSelected: (val) =>
                    setState(() => _desiredFrequency = val ? e.key : null),
              );
            }).toList(),
          ),
        ],
  ...
  Widget _buildInjuriesStep() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Salud y Preferencias',
              style: Theme.of(context).textTheme.headlineSmall),
          const SizedBox(height: 24),
          const Text('¿Tenés lesiones o molestias?',
              style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          const SizedBox(height: 24),
          const Text('¿Puede recostarse y levantarse por sus propios medios?',
              style: TextStyle(fontWeight: FontWeight.bold)),
          Row(
            children: [
              Expanded(
                  child: RadioListTile<bool>(
                      title: const Text('Sí'),
                      value: true,
                      groupValue: _canLieDown,
                      onChanged: (v) => setState(() => _canLieDown = v))),
              Expanded(
                  child: RadioListTile<bool>(
                      title: const Text('No'),
                      value: false,
          ...
          const Text('¿Puede arrodillarse y levantarse por sus propios medios?',
              style: TextStyle(fontWeight: FontWeight.bold)),
          Row(
            children: [
              Expanded(
                  child: RadioListTile<bool>(
                      title: const Text('Sí'),
                      value: true,
                      groupValue: _canKneel,
                      onChanged: (v) => setState(() => _canKneel = v))),
              Expanded(
                  child: RadioListTile<bool>(
                      title: const Text('No'),
                      value: false,
          ...
          const SizedBox(height: 32),
          const Text('¿Algo más que debamos saber?',
              style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          TextField(
