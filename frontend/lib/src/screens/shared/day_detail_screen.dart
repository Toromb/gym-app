...
class _DayDetailScreenState extends State<DayDetailScreen> {
      ...
      if (!widget.readOnly) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          if (widget.freeTrainingId != null) {
            context.read<PlanProvider>().startSession(null, null, null,
            ...
            context.read<PlanProvider>().startSession(
                widget.planId, widget.weekNumber, widget.day!.order);
          }
        });
      ...
      if (hasIncomplete) {
        final confirm = await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text(
                'Hay ejercicios sin completar. ¿Estás seguro de que deseas finalizar?'),
            actions: [
              TextButton(
    ...
    if (mounted) {
      ...
      try {
        ...
        if (currentSession != null) {
          await context
              .read<PlanProvider>()
              .completeSession(dateStr, dayId: widget.day?.id);
        }
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    AppLocalizations.of(context)!.get('workoutFinished'),
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.readOnly) {
      if (_readOnlySession != null) {
        ...
        exercisesToRender = widget.day!.exercises
            .map((pe) => SessionExercise.fromPlanExercise(pe))
            .toList();
        status = 'SIN DATOS';
      }
    } else {
                ...
                ElevatedButton(
                    onPressed: () {
                      if (widget.freeTrainingId != null) {
                        ...
                        context.read<PlanProvider>().startSession(widget.planId,
                            widget.weekNumber, widget.day!.order);
                      }
                    },
                  ...
                  children: [
                    ...
                    ...exercisesToRender.asMap().entries.map((entry) {
                      ...
                      return _buildExerciseCard(
                          context, exExec, index + 1, widget.readOnly);
                    }),

                    // Fix: Hide Add Exercise button for Plan Sessions
                    if (!widget.readOnly &&
                        (widget.planId == 'FREE_SESSION' ||
                            widget.freeTrainingId != null))
                      Padding(
            ...
            ? SafeArea(
                child: Padding(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(
                        AppLocalizations.of(context)!.get('finishWorkout')),
  ...
  Widget _buildExerciseCard(
      BuildContext context, SessionExercise exExec, int index, bool readOnly) {
