...
class _GymAdminsScreenState extends State<GymAdminsScreen> {
  ...
  void _fetchAdmins() {
    context
        .read<UserProvider>()
        .fetchUsers(role: AppRoles.admin, gymId: _selectedGymId);
  }
          ...
          IconButton(
              ...
              onPressed: () async {
                await Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            const AddUserScreen(lockedRole: AppRoles.admin)));
                _fetchAdmins();
              }),
        ],
      ),
          ...
          Expanded(
            child: Consumer<UserProvider>(
              builder: (context, provider, child) {
                ...
                final admins = provider.students
                            ...
                            onSelected: (value) async {
                              ...
                              if (value == 'activation') {
                                token = await authService
                                    .generateActivationToken(admin.id);
                              } else if (value == 'reset') {
                                token = await authService
                                    .generateResetToken(admin.id);
                              }
                            ...
                            onPressed: () async {
                              final confirm = await showDialog<bool>(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: Text(
                                      '¿Está seguro que desea eliminar al administrador ${admin.name}?'),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
