...
async function run() {
  ...
  try {
    ...
    console.log(
      `✅ Teacher Logged In (ID: ${teacherId}, Gym: ${gymId || 'None'})`,
    );
    ...
    if (fetchedExercises.length < 2)
      throw new Error('Not enough exercises in DB to run test');
    ...
    console.log(
      `✅ Execution Started: ${executionId} with ${exercises.length} exercises`,
    );
    ...
    for (const ex of exercises) {
      await axios.patch(
        `${BASE_URL}/executions/exercises/${ex.id}`,
        {
          isCompleted: true,
          setsDone: '3',
          repsDone: '10',
    ...
    console.log('\n🏁 9. Finish Workout...');
    await axios.patch(
      `${BASE_URL}/executions/${executionId}/complete`,
      {
        date: today,
      },
      { headers: { Authorization: `Bearer ${studentToken}` } },
    ...
    const assignmentsRes = await axios.get(
      `${BASE_URL}/plans/student/history`,
      {
        headers: { Authorization: `Bearer ${studentToken}` },
      },
    );
    ...
    if (assignment.progress.days[dayId]?.completed === true) {
      ...
      console.error(
        '❌ Legacy Progress Sync FAILED',
        JSON.stringify(assignment.progress, null, 2),
      );
