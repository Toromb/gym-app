...
class _CreatePlanScreenState extends State<CreatePlanScreen> {
  ...
  void initState() {
    ...
    _durationController =
        TextEditingController(text: plan?.durationWeeks.toString() ?? '4');
                    ...
                    .map((d) => PlanDay(
                          ...
                          exercises: d.exercises
                              .map((e) => PlanExercise(
                                    ...
                                    equipments:
                                        e.equipments, // Preserve equipments
                                  ))
                              .toList(),
                        ))
                    .toList(),
  ...
  void _removeExercise(int weekIndex, int dayIndex, int exerciseIndex) {
    setState(() {
      ...
      // Re-order
      for (int i = 0;
          i < _weeks[weekIndex].days[dayIndex].exercises.length;
          i++) {
  ...
  void _addOrEditExercise(int weekIndex, int dayIndex,
      [PlanExercise? existingExercise, int? exerciseIndex]) async {
    ...
    if (exercises.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('No hay ejercicios disponibles. Crea algunos primero.')),
      );
      return;
    }
    ...
    if (existingExercise?.exerciseId != null) {
      try {
        selectedExercise =
            exercises.firstWhere((e) => e.id == existingExercise!.exerciseId);
      } catch (_) {}
    } else if (existingExercise?.exercise != null) {
      try {
        selectedExercise =
            exercises.firstWhere((e) => e.id == existingExercise!.exercise!.id);
      } catch (_) {}
    }

    final setsController =
        TextEditingController(text: existingExercise?.sets.toString() ?? '');
    final repsController =
        TextEditingController(text: existingExercise?.reps ?? '');
    final loadController =
        TextEditingController(text: existingExercise?.suggestedLoad ?? '');
    final restController =
        TextEditingController(text: existingExercise?.rest ?? '');

    // New Controllers
    final timeController = TextEditingController(
        text: existingExercise?.targetTime?.toString() ?? '');
    final distanceController = TextEditingController(
        text: existingExercise?.targetDistance?.toString() ?? '');

    final notesController =
        TextEditingController(text: existingExercise?.notes ?? '');
    final videoUrlController = TextEditingController(
        text: existingExercise?.videoUrl ??
            existingExercise?.exercise?.videoUrl ??
            '');

    // Equipment Selection State
    List<Equipment> selectedPlanEquipments =
        existingExercise?.equipments.toList() ?? [];
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                DropdownButtonFormField<Exercise>(
                  ...
                  onChanged: (value) {
                    setStateDialog(() {
                      ...
                      if (value != null) {
                        setsController.text = value.sets?.toString() ??
                ...
                if (selectedExercise != null &&
                    selectedExercise!.equipments.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Align(
                      alignment: Alignment.centerLeft,
                      child: Text('Equipamiento/s en este ejercicio',
                          style: TextStyle(
                  ...
                  Wrap(
                    ...
                    children: selectedExercise!.equipments.map((eq) {
                      final isSelected =
                          selectedPlanEquipments.any((s) => s.id == eq.id);
                      return FilterChip(
                        label: Text(eq.name),
                        selected: isSelected,
                ...
                if (metricType == 'REPS') ...[
                  ...
                  Builder(builder: (context) {
                    ...
                    if (selectedPlanEquipments.any((e) => e.isBodyWeight)) {
                      isBodyWeight = true;
                    } else if (selectedPlanEquipments.isEmpty &&
                    ...
                    return TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          helperText: isBodyWeight
                              ? 'Se sumará al peso corporal'
                              : null,
                        ));
                  }),
                  ...
                  TextField(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Tiempo Objetivo (segundos)'),
                      keyboardType: TextInputType.number),
                ] else if (metricType == 'DISTANCE') ...[
                  TextField(
                      controller: distanceController,
                      decoration: const InputDecoration(
                          labelText: 'Distancia Objetivo (metros)'),
                      keyboardType: TextInputType.number),
                ],
                ...
                TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'URL de Video')),
                TextField(
                ...
                if (selectedExercise != null) {
                  setState(() {
                    final newExercise = PlanExercise(
                      ...
                      suggestedLoad:
                          metricType == 'REPS' ? loadController.text : null,

                      targetTime: metricType == 'TIME'
                          ? int.tryParse(timeController.text)
                          : null,
                      targetDistance: metricType == 'DISTANCE'
                      ...
                      order: existingExercise?.order ??
                          _weeks[weekIndex].days[dayIndex].exercises.length + 1,
