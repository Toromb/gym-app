import { Controller, Get, Post, Patch, Body, Param, UseGuards, Request, Query, BadRequestException, ForbiddenException } from '@nestjs/common';
...
export class ExecutionsController {
    ...
    async startExecution(
        @Request() req: any,
        @Body() body: { planId: string; weekNumber: number; dayOrder: number; date: string }
    ) {
    ...
    async updateExercise(
        ...
        return this.executionsService.updateExercise(
            exerciseExecId,
            body
        );
    }
    ...
    async completeExecution(
        ...
        if (req.user.role !== UserRole.ALUMNO && req.user.role !== UserRole.PROFE) {
            throw new ForbiddenException('Only Student or Professor can complete executions');
        }

        if (!body.date) throw new BadRequestException('Date is required to complete execution');
    ...
    @Get('calendar')
    async getCalendar(@Request() req: any, @Query('from') from: string, @Query('to') to: string) {
        if (!from || !to) throw new BadRequestException('from and to dates required');
    ...
    async getExecutionByStructure(
        ...
        // Allowed: Profe, Admin
        if (req.user.role !== UserRole.PROFE && req.user.role !== UserRole.ADMIN && req.user.role !== UserRole.SUPER_ADMIN) {
