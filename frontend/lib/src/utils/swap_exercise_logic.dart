  ...
  static SwapSuggestion calculate({
    ...
    if (oldExercise.metricType != newExercise.metricType) {
      return SwapSuggestion(
        ...
        warning:
            'El ejercicio tiene un tipo de medición diferente (${newExercise.metricType}).',
      );
    }
    ...
    if (newExercise.metricType == 'TIME') {
      ...
      final oldTime =
          execution.timeSpent ?? execution.targetTimeSnapshot?.toString();
      if (oldTime != null) {
        newTime = oldTime;
        // Clamp if needed
      ...
      String? newDist;
      final oldDist = execution.distanceCovered?.toString() ??
          execution.targetDistanceSnapshot?.toString();
      if (oldDist != null) {
        newDist = oldDist;
      } else {
    ...
    if (oldWeightStr != null && oldWeightStr.isNotEmpty) {
      oldExternalLoad =
          double.tryParse(oldWeightStr.replaceAll(RegExp(r'[^0-9.]'), ''));
    }
    ...
    final isBodyweight = oldExercise.equipments.any((e) =>
        e.name.toUpperCase().contains('BODYWEIGHT') ||
        e.id.toUpperCase().contains('BODYWEIGHT'));
    ...
    if (referenceWeight != null &&
        oldExercise.loadFactor != null &&
        newExercise.loadFactor != null) {
    ...
    if (intent == TrainingIntent.GENERAL) {
      ...
      if (effectiveMin <= effectiveMax) {
        if (oldRepsVal != null &&
            oldRepsVal >= effectiveMin &&
            oldRepsVal <= effectiveMax) {
          newReps = oldRepsVal.toString();
        } else {
          if (effectiveMin == effectiveMax) {
