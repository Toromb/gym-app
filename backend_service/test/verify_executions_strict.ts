...
async function main() {
  ...
  try {
    ...
    if (exec1.exercises.length === 0)
      throw new Error('Execution has no exercises');
    ...
    if (exec1Re.id !== exec1.id)
      throw new Error('Re-entry did not return same execution ID');
    ...
    await patch(
      `/executions/${exec1.id}/complete`,
      { date: '2025-01-01' },
      studToken,
    );
    const exec1Completed = await get(`/executions/${exec1.id}`, studToken);
    if (exec1Completed.status !== 'COMPLETED')
    ...
    if (exec2.id === exec1.id)
      throw new Error('Should have created NEW execution for new date');
    console.log(`   Execution 2 started: ${exec2.id}`);

    // 7. Conflict Test: Try to complete exec2 with date 2025-01-01
    console.log(
      '7. Testing Date Conflict (Try to set Exec 2 to 2025-01-01)...',
    );
    try {
      await patch(
        `/executions/${exec2.id}/complete`,
        { date: '2025-01-01' },
        studToken,
      );
    ...
    const calendar = await get(
      '/executions/calendar?from=2025-01-01&to=2025-01-31',
      studToken,
    );
    ...
    if (calendar.length !== 1)
      throw new Error(`Calendar should have 1 item, found ${calendar.length}`);
