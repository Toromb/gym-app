...
class FreeTrainingSelectorScreen extends StatefulWidget {
  ...
  final Function(FreeTraining)?
      onTrainingSelected; // For Admin "Edit" or generic selection
  ...
  @override
  State<FreeTrainingSelectorScreen> createState() =>
      _FreeTrainingSelectorScreenState();
}

class _FreeTrainingSelectorScreenState
    extends State<FreeTrainingSelectorScreen> {
  ...
  Future<void> _search() async {
    ...
    try {
      final results = await _service.getFreeTrainings(
        ...
        level:
            _selectedLevel != null ? _toBackendFormat(_selectedLevel!) : null,
        sector:
            _selectedSector != null ? _toBackendFormat(_selectedSector!) : null,
        cardioLevel: _selectedCardioLevel != null
            ? _toBackendFormat(_selectedCardioLevel!)
            : null,
      );
      setState(() {
        _results = results;
  ...
  void _nextPage() {
    ...
    if (_currentStep == 1) {
      ...
      if (_selectedType == FreeTrainingType.musculacion ||
          _selectedType == FreeTrainingType.funcional) {
  ...
  void _prevPage() {
    ...
    if (_currentStep == 4) {
      ...
      if (_selectedType == FreeTrainingType.cardio)
        prev = 1;
      else if (_selectedType == FreeTrainingType.musculacion ||
          _selectedType == FreeTrainingType.funcional)
        prev = 2;
  ...
  Widget _buildSectorStep(ThemeData theme) {
    ...
    return ListView(padding: const EdgeInsets.all(24), children: [
      ...
      ...BodySector.values.map((s) {
        ...
        IconData icon;
        switch (s) {
          case BodySector.piernas:
            color = Colors.deepOrange;
            icon = Icons.directions_walk;
            break;
          case BodySector.zonaMedia:
            color = Colors.pink;
            icon = Icons.accessibility_new;
            break; // Abs/Core
          case BodySector.hombros:
            color = Colors.indigo;
            icon = Icons.emoji_people;
            break; // Shoulders
          case BodySector.espalda:
            color = Colors.blue;
            icon = Icons.rowing;
            break; // Back -> Rowing is good
          case BodySector.pecho:
            color = Colors.red;
            icon = Icons.shield;
            break; // Chest -> Shield (Protection/Torso)
          case BodySector.fullBody:
            color = Colors.teal;
        ...
        return Padding(
          ...
          child: _buildSelectionCard(theme,
  ...
  Widget _buildCardioStep(ThemeData theme) {
    ...
    return ListView(padding: const EdgeInsets.all(24), children: [
      ...
      ...CardioLevel.values.map((c) {
        ...
        IconData icon;
        switch (c) {
          case CardioLevel.inicial:
            color = Colors.teal;
            icon = Icons.directions_walk;
            break;
          case CardioLevel.medio:
            color = Colors.amber.shade800;
            icon = Icons.directions_run;
            break;
          case CardioLevel.avanzado:
            color = Colors.red.shade700;
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            FilledButton.tonal(
                onPressed: _reset, child: const Text('NUEVA BÚSQUEDA'))
          ],
        ),
      );
    }
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                      '${t.level.name.toUpperCase()} • ${t.type.name.toUpperCase()}',
                                ...
                                onPressed: () async {
                                  ...
                                  if (confirm == true) {
                                    try {
                                      ...
                                      await _service.deleteFreeTraining(t
                                          .id); // Need to implement delete in service!
                                      // Refresh
                                      _search();
                                      if (mounted)
                                        ScaffoldMessenger.of(context)
