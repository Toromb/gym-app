...
async function verifySuperAdmin() {
    ...
    if (!saLogin.ok) throw new Error(`SA Login failed: ${JSON.stringify(saData)}`);
    ...
    if (!adminRes.ok) throw new Error(`Create Gym Admin failed: ${JSON.stringify(adminUser)}`);
    ...
    if (!adminLogin.ok) throw new Error(`Gym Admin Login failed: ${JSON.stringify(adminData)}`);
    ...
    if (!profeRes.ok) throw new Error(`Create Profe failed: ${JSON.stringify(profeUser)}`);
