...
class _ManageUsersScreenState extends State<ManageUsersScreen> {
  ...
  void _showInviteLinkDialog(BuildContext context) {
    ...
    if (gymId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('Error: No se encontró el gimnasio del administrador.')));
      return;
    }
  ...
  List<Widget> _buildUserListWidgets(BuildContext context, List<dynamic> users,
      bool isAdmin, bool isProfeView) {
    if (users.isEmpty) {
      return [
        const Padding(
                    ...
                    if ((isProfeView || isAdmin) && user.role == 'alumno') ...[
                      if (isAdmin) ...[
                        IconButton(
                          ...
                          onPressed: () =>
                              _showAssignProfessorDialog(context, user),
                        ),
                        const SizedBox(width: 12),
                      ],
                      IconButton(
                        ...
                        onPressed: () =>
                            _showAssignPlanDialog(context, user.id),
                      ),
                      const SizedBox(width: 12),
                      IconButton(
                        visualDensity: VisualDensity.compact,
                        ...
                        onPressed: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  StudentPlansScreen(student: user),
                            ),
                          );
                        },
                      ),
                      ...
                      PopupMenuButton<String>(
                        ...
                        onSelected: (value) async {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text('Generando enlace...'),
                                duration: Duration(seconds: 1)),
                          );

                          if (value == 'activation') {
                            token = await authService
                                .generateActivationToken(user.id);
                          } else if (value == 'reset') {
                            token =
                                await authService.generateResetToken(user.id);
                          }
                          ...
                          if (token != null && context.mounted) {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context)
                                  .removeCurrentSnackBar(); // Remove "Generating..."
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content:
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text('Error al generar enlace'),
              ...
              onPressed: () async {
                if (selectedPlanId != null) {
                  final error = await context
                      .read<PlanProvider>()
                      .assignPlan(selectedPlanId!, studentId);
                  if (context.mounted) {
...
class _AssignProfessorDialogState extends State<_AssignProfessorDialog> {
  ...
  Future<void> _fetchProfessors() async {
    try {
      ...
      final professors =
          allUsers.where((u) => u.role == UserRoles.profe).toList();
      if (mounted) {
        setState(() {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              } // If null, backend handles unassign
                  );
              if (context.mounted) {
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _generateLink() async {
    try {
      ...
      final token = await authService
          .generateInviteLink(widget.gymId); // Defaults to ALUMNO
          ...
          children: [
            ...
            if (_isLoading)
              ...
              Column(
                ...
                children: [
                  const Text(
                      'Comparte este QR o enlace con tus alumnos para que se registren automáticamente a tu gimnasio.',
                      textAlign: TextAlign.center,
