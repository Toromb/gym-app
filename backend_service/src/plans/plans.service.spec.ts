...
describe('PlansService', () => {
    ...
    const mockPlanRepository = {
        save: jest.fn().mockImplementation(plan => Promise.resolve({ id: 'plan-id', ...plan })),
        findOne: jest.fn(),
        find: jest.fn(),
    };

    const mockStudentPlanRepository = {
        save: jest.fn().mockImplementation(sp => Promise.resolve({ id: 'sp-id', ...sp })),
        update: jest.fn(),
    };
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(StudentPlan), useValue: mockStudentPlanRepository },
    ...
    it('should create a plan with deep structure', async () => {
        ...
        const dto = {
            ...
            durationWeeks: 4,
            weeks: [
                { weekNumber: 1, days: [{ dayOfWeek: 1, exercises: [] }] }
            ]
        };
