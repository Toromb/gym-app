...
class RegisterWithInviteScreen extends StatefulWidget {
  ...
  @override
  _RegisterWithInviteScreenState createState() =>
...
class _RegisterWithInviteScreenState extends State<RegisterWithInviteScreen> {
      ...
      if (response != null && response['access_token'] != null) {
        ...
        await showDialog(
            ...
            builder: (ctx) => AlertDialog(
                  ...
                  content: const Text(
                      'Tu cuenta ha sido creada y vinculada al gimnasio. Por favor inicia sesión.'),
                  actions: [
                    TextButton(
                        onPressed: () {
  ...
  Future<void> _performGoogleLogin() async {
    ...
    final errorMsg = await context
        .read<AuthProvider>()
        .loginWithGoogle(inviteToken: widget.inviteToken);

    if (!mounted) return;

    if (errorMsg == null) {
      Navigator.of(context)
          .pop(); // Should redirect home from AuthProvider state change
    } else {
  ...
  Future<void> _performAppleLogin() async {
    ...
    final errorMsg = await context
        .read<AuthProvider>()
        .loginWithApple(inviteToken: widget.inviteToken);
